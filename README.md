...
## LPRW 4.0 — Xray data-plane

LPRW 4.x separates the web panel from the traffic data-plane. The panel manages
inbounds and clients while Xray-core handles the actual forwarding, which is
substantially more appropriate for high-throughput VPS traffic than relaying
payloads through Python/ASGI.

### Supported configuration

- Protocols: VLESS, Trojan, Shadowsocks
- Transports: WebSocket, XHTTP, HTTPUpgrade
- Security: none or TLS
- Reality is intentionally not exposed by the UI.
- XHTTP mode: auto, packet-up, stream-up

Shadowsocks is generated as a native Shadowsocks TCP/UDP server. WS/XHTTP/
HTTPUpgrade are not native Shadowsocks transports, so the panel does not
pretend that they are; SS links remain interoperable with standard clients.

### VPS environment

For direct TLS termination by Xray, set:

```bash
export XRAY_TLS_CERT=/etc/letsencrypt/live/example.com/fullchain.pem
export XRAY_TLS_KEY=/etc/letsencrypt/live/example.com/privkey.pem
export XRAY_VERSION=26.7.28
export INBOUND_PORT_START=20000
```

Open the generated inbound ports in the VPS firewall/security group.

If TLS is terminated by Nginx/Caddy/another reverse proxy, use `security=none`
on the Xray inbound and proxy the chosen WS/XHTTP/HTTPUpgrade path to that
inbound port.

The first Xray start downloads the configured Xray-core release into
`DATA_DIR/xray`. If the runtime cannot reach GitHub, place an executable Xray
binary at `XRAY_PATH` (default `DATA_DIR/xray/xray`) and the panel will use it.

### Migration

Existing LPRW 3.x links are automatically migrated into generated inbounds.
New links are created by selecting an existing inbound; protocol/transport
settings are no longer duplicated on each link.

### Performance

The Python relay remains available for the legacy `/ws/{uuid}` and
`/trojan-ws/{uuid}` endpoints, but new links use Xray-core. This removes the
Python message-loop bottleneck from normal VLESS/Trojan/SS traffic. Actual
latency and throughput still depend on the VPS CPU/network, client, route and
TLS termination.
